        ...
        <div> ${openClass.className} <c:if test="${openClass.classType == 1}"><span id="ep-count">(<c:if test="${openCount > 0}">${openCount}</c:if><c:if test="${openCount == 0}">1</c:if>회차)</span></c:if> <c:if test="${openClass.classType != 1}"><span id="ep-count">(원데이)</span></c:if>       <input type="hidden" name="memberNo" value="${openClass.classNo}"/> </div>    
            ...
            <div id="c-ptn">
                ...
                <c:if test="${openClass.classType == 1}">
	                <div id="ptn-multi">
	                    <div>시간당 가격</div>
	                    <div><input type="number" id="time-price" name="timePrice" class="input-style largeSize" value="${timePrice}" required min="0"> &nbsp원</div>
	                    <div>1회당 수업시간</div>
	                    <div><input type="number" id="num-time" name="schdlTime" class="input-style largeSize" value="${schdlTime}" required min="0" max="15"> &nbsp시간</div>
	                    <div>총 수업횟수</div>
